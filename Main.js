...
var $lovasoa$elm_csv$Csv$Csv = F2(
...
var $author$project$Main$subscriptions = function (model) {
	return $author$project$Ports$fileContentRead($author$project$Main$CSVParse);
};
	...
	function (msg, model) {
		switch (msg.$) {
			case 'CSVSelected':
				return _Utils_Tuple2(
					model,
					$author$project$Ports$fileSelected('CSVInput'));
			case 'CSVParse':
				var data = msg.a;
				var newCSVFile = {contents: data.contents, filename: data.filename};
				var _v1 = $lovasoa$elm_csv$Csv$parse(newCSVFile.contents);
				var results = _v1;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							csvData: results,
							csvFile: $elm$core$Maybe$Just(newCSVFile)
						}),
					$elm$core$Platform$Cmd$none);
			case 'Change':
				var content = msg.a;
...
var $author$project$Main$csvTable = function (data) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
...
var $author$project$Main$csvView = F2(
	function (file, csvData) {
		if (file.$ === 'Just') {
			var i = file.a;
			return $author$project$Main$csvTable(csvData);
		} else {
...
var $author$project$Main$findOptionsGiven = F2(
	function (selector, csvData) {
		var names = A2(
			$elm$core$List$map,
			function (r) {
				return $author$project$Main$extractName(r);
			},
			csvData.records);
		var matching = $elm$core$Set$fromList(
			A2(
				$elm$core$List$filter,
				function (x) {
					return A2($elm$core$String$startsWith, selector, x);
				},
				names));
		return $elm$core$Set$toList(matching);
	});
var $author$project$Main$ToggleSelection = function (a) {
	return {$: 'ToggleSelection', a: a};
};
...
var $author$project$Main$renderOption = function (item) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
					...
					[
						...
						_List_fromArray(
							[
								...
								$author$project$Main$ToggleSelection(item))
							]),
						_List_Nil),
...
var $author$project$Main$viewPersonOptions = function (model) {
	var options = A2($author$project$Main$findOptionsGiven, model.personSelected, model.csvData);
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		A2(
			$elm$core$List$map,
			function (x) {
				return $author$project$Main$renderOption(x);
			},
			options));
};
var $author$project$Main$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('FileWrapper')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$id('CSVInput'),
						A2(
						$elm$html$Html$Events$on,
						'change',
						$elm$json$Json$Decode$succeed($author$project$Main$CSVSelected))
					]),
				_List_Nil),
				A2($author$project$Main$csvView, model.csvFile, model.csvData),
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$placeholder('Select person'),
						$elm$html$Html$Attributes$value(model.personSelected),
				...
				_List_fromArray(
					[
						$elm$html$Html$text(model.personSelected)
					])),
				$author$project$Main$viewPersonOptions(model),
				A2(
				$elm$html$Html$h2,
				_List_Nil,
				_List_fromArray(
